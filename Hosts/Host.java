...
// CAPA 3: RED
    ...
    // Recopilar todos los paquetes para mostrarlos
    ...
    VisualizadorOSI.imprimirPanel("1 - FISICA", "Envío", 
        ...
        "Lista de " + p2.size() + " Tramas", "Formato Binario", "Archivo: " + nombreArchivo);
    ...
    try {
        ...
        for (Trama tr : p1) { tramasFisica.append(tr.getCompleto()).append("\n"); }
        ...
        for (Paquete pq : p2) { paquetesEnlace.append(pq.getCompleto()).append("\n"); }
        
        VisualizadorOSI.imprimirPanel("2 - ENLACE", "Recepción", 
            "Verificación de integridad matemática y eliminacion de cabecera MAC.", 
            tramasFisica.toString().trim(), p1.get(0).getCabecera() + " (Cabeceras MAC)", paquetesEnlace.toString().trim());

        // CAPA 3: RED
        List<Segmento> p3 = capa3.desencapsular(p2);
        
        StringBuilder segmentosRed = new StringBuilder();
        for (Segmento seg : p3) { segmentosRed.append(seg.getCompleto()).append("\n"); }
        
        VisualizadorOSI.imprimirPanel("3 - RED", "Recepción", 
            "Eliminacion de cabeceras IP ", 
            paquetesEnlace.toString().trim(), p2.get(0).getCabecera() + " (Cabeceras IP)", segmentosRed.toString().trim());
        ...
        // Como le quitamos los "System.out.println" a las capas, si alguna tira un error
        // (por ejemplo, si "PC-Intruso" intenta abrir un mensaje que no es suyo en la Capa 5),
